...
#define NULL_CHECK_AND_RETURN(data) \
        if (data == NULL) { \
            ASN_FREE_EX(&msgObj); \
            return; \
        }

#define NULL_CHECK_AND_RETURN_BOOL(data, bool) \
        if (data == NULL) { \
            ASN_FREE_EX(&msgObj); \
            return bool; \
        }
...
#define FILL_FIELD_VALUE(val, type, field)  \
    ...
    if (val != NULL) { \
        *(val) = m_field_value[field]; \
        ...
        if (TRANSLATE_WITH_ASN_DOT_1_ENCODE) { \
            AsnEncoder::FreeAsnBuf(m_pAsnDescriptor, &msgObj); \
            DMC_LOGE("DMC-TranslatorUtils", "OOM to calloc %s!", #val); \
        } \
        return; \
    }

#define FILL_FIELD_VALUE_ERR_NOT_RET(val, type, field, err)  \
    val = (type*) calloc(1, sizeof(type)); \
    if (val != NULL) { \
        *(val) = m_field_value[field]; \
        err = false; \
